...
void setup() {
  ...
  // if (ret != 0) {
  //     SPDLOG_ERROR("Failed to set thread scheduling policy for worker {}.
  //     Error: {}", worker.id(), strerror(ret)); SPDLOG_WARN("You may need to
  //     run with sudo or grant CAP_SYS_NICE capabilities.");
  // } else {
  //     SPDLOG_INFO("Successfully set worker {} scheduling policy to SCHED_FIFO
  //     with priority {}", worker.id(), params.sched_priority);
  // }
  ...
  // if (ret != 0) {
  //     SPDLOG_ERROR("Failed to set CPU affinity for worker {}. Error: {}",
  //     worker.id(), strerror(ret));
  // } else {
  //     SPDLOG_INFO("Successfully pinned worker {} to CPU {}", worker.id(),
  //     core_id);
  // }
}
...
int main() {
  ...
  setup();
  // Check for root privileges, which are required to access the driver's sysfs
  // files if (geteuid() != 0) {
  //     std::cerr << "Error: This program requires root privileges to read from
  //     sysfs." << std::endl; std::cerr << "Please run with sudo." <<
  ...
  try {
    ...
    if (pm_table_size == 0 || pm_table_size > 16384) { // Sanity check size
      std::cerr << "Error: Invalid pm_table size reported: " << pm_table_size
                << " bytes." << std::endl;
      return EXIT_FAILURE;
    }
    std::cout << "Detected pm_table size: " << pm_table_size << " bytes."
              << std::endl;

    // Open the pm_table file for reading
    std::ifstream pm_table_stream(PM_TABLE_PATH, std::ios::binary);
    if (!pm_table_stream) {
      std::cerr << "Error: Failed to open " << PM_TABLE_PATH << "."
                << std::endl;
    ...
    if (!output_stream) {
      std::cerr << "Error: Failed to open output file " << OUTPUT_FILE_PATH
                << " for writing." << std::endl;
      return EXIT_FAILURE;
    }

    std::cout << "Starting to read pm_table at 1kHz. Press Ctrl+C to stop."
              << std::endl;
    ...
    while (running) {
      // Calculate the time for the next iteration to maintain a consistent 1kHz
      // rate
      ...
      if (!pm_table_stream) {
        std::cerr << "\nWarning: Failed to read from " << PM_TABLE_PATH
                  << " on sample " << samples_written << std::endl;
      ...
      output_stream.write(reinterpret_cast<const char *>(&timestamp_u64),
                          sizeof(timestamp_u64));
      output_stream.write(reinterpret_cast<const char *>(&pm_table_size),
                          sizeof(pm_table_size));
    ...
    // --- Cleanup ---
    std::cout << "Stopped. Wrote " << samples_written << " samples to "
