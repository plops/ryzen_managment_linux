...
RealtimeGuard::RealtimeGuard(int core_id, int priority,
                             bool lock_memory) noexcept
    : active_(false), locked_memory_(false), core_id_(core_id),
      new_priority_(priority), old_policy_(SCHED_OTHER),
  ...
  if (core_id_ >= 0) {
    ...
    if (ret != 0) {
      SPDLOG_WARN("pthread_setaffinity_np failed to pin to core {}: {}",
                  core_id_, std::strerror(errno));
    }
  }
  ...
  if (ret != 0) {
    SPDLOG_WARN("pthread_setschedparam(SCHED_FIFO) failed: {}. You may need "
                "root / CAP_SYS_NICE.",
                std::strerror(errno));
  ...
  if (lock_memory) {
    // Check RLIMIT_MEMLOCK first to avoid pointless mlockall calls that will
    // fail or cause OOM.
    struct rlimit rl;
    if (getrlimit(RLIMIT_MEMLOCK, &rl) == 0) {
      if (rl.rlim_cur == 0) {
        SPDLOG_WARN("RLIMIT_MEMLOCK is 0: skipping mlockall. Consider "
                    "increasing memlock limit or using per-buffer mlock.");
      } else {
        if (mlockall(MCL_CURRENT | MCL_FUTURE) != 0) {
          SPDLOG_WARN(
              "mlockall failed: {}. Avoid calling mlockall on large processes; "
              "prefer page-aligned per-buffer mlock (CAP_IPC_LOCK needed).",
              std::strerror(errno));
        } else {
          locked_memory_ = true;
        }
      ...
      if (mlockall(MCL_CURRENT | MCL_FUTURE) != 0) {
        SPDLOG_WARN("mlockall failed and getrlimit() failed as well: {}. "
                    "Prefer per-buffer locking to avoid OOM.",
                    std::strerror(errno));
      } else {
        locked_memory_ = true;
      }
...
RealtimeGuard::~RealtimeGuard() noexcept {
  ...
  if (ret != 0) {
    SPDLOG_WARN("Failed to restore thread scheduling: {}",
                std::strerror(errno));
  }
  ...
  if (saved_affinity_) {
    ...
    if (ret != 0) {
      SPDLOG_WARN("Failed to restore thread affinity: {}",
                  std::strerror(errno));
    }
  }
