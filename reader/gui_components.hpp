...
struct EyePlotData {
  ...
  std::vector<float> y_data; // Trimmed mean values
};

/**
 * @brief Manages the data preparation for the GUI.
 *
 * This class is responsible for periodically processing the raw
 * EyeDiagramStorage into a render-ready format (calculating trimmed means).
 * This avoids doing heavy computation in the render loop.
 */
class GuiDataCache {
public:
   ...
   * It locks the eye_storage, calculates trimmed means, and updates its
   * internal cache.
 ...
 * @param interesting_indices A map from original sensor index to its position
 * in the cache.
 * @param experiment_status A string describing the current state of the
 * experiment.
 * @param manual_mode Atomic bool to control auto vs manual mode.
 * @param manual_core_to_test Atomic int for the user-selected core in manual
 * mode.
 * @param num_hardware_threads Total number of threads for the core selector
 * range.
 */
