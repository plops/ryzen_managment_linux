 ...
 */
EyeDiagramStorage::EyeDiagramStorage(std::span<const int> interesting_indices,
                                     size_t reserve_per_bin)
    : EyeDiagramStorage(interesting_indices, reserve_per_bin,
                        DEFAULT_WINDOW_BEFORE_MS, DEFAULT_WINDOW_AFTER_MS) {}

/**
 * @brief Construct EyeDiagramStorage, set custom window parameters, and
 * allocate.
 *
 * This prevents reallocations during capture if reserve_per_bin is chosen
 * appropriately. It also stores the mapping from internal storage index to
 * original sensor index.
 */
EyeDiagramStorage::EyeDiagramStorage(std::span<const int> interesting_indices,
                                     size_t reserve_per_bin,
                                     int window_before_ms,
                                     int window_after_ms) {
  ...
  this->original_sensor_indices.assign(interesting_indices.begin(),
                                       interesting_indices.end());
