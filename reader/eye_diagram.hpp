 ...
 *
 * - bins[bin_index][sensor_index] is a vector<float> of samples falling into
 * that time bin.
...
struct EyeDiagramStorage {
  ...
  // bins[storage_index][bin_index] => vector<float> of values for that bin &
  // sensor Note the swapped index order for better memory access patterns.
   ...
   */
  EyeDiagramStorage(std::span<const int> interesting_indices,
   ...
   * @param interesting_indices The original indices of sensors to track.
   * @param reserve_per_bin Expected number of samples per sensor per bin.
   * @param window_before_ms Look-back window in milliseconds (defines zero
   * offset).
   * @param window_after_ms Look-forward window in milliseconds.
   */
  EyeDiagramStorage(std::span<const int> interesting_indices,
                    size_t reserve_per_bin, int window_before_ms,
                    int window_after_ms);
