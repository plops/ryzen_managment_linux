 ...
 */
EyeCapturer::EyeCapturer(EyeDiagramStorage &storage) : storage_(storage) {
  // Build the reverse map for O(1) lookup of storage index from original sensor
  // index
...
/**
 * @brief Process a sample and bin sensor values relative to the most recent
 * rising edge.
 *
 ...
 */
bool EyeCapturer::process_sample(const TimePoint &timestamp, int worker_state,
  ...
  if (state_ == State::CAPTURING) {
    ...
    long long bin_index =
        time_delta_ms + static_cast<long long>(storage_.zero_offset_bins);

    if (bin_index >= 0 &&
        bin_index < static_cast<long long>(storage_.num_bins)) {
      // For each sensor in the measurement, check if it's one we should store.
      for (size_t sensor_idx = 0; sensor_idx < measurements.size();
           ++sensor_idx) {
        ...
        if (it != sensor_to_storage_idx_.end()) {
          ...
          storage_.bins[storage_idx][bin_index].push_back(
              measurements[sensor_idx]);
        }
      }
