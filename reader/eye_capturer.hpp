 ...
 * @brief State machine that detects worker rising edges and fills
 * EyeDiagramStorage.
 */
 ...
 *
 * On detecting a rising edge (worker state 0->1) capturing starts and
 * subsequent samples are binned relative to the rise time into
 * EyeDiagramStorage until the capture window end is reached.
 */
class EyeCapturer {
public:
  /**
   * @brief Construct an EyeCapturer bound to a pre-configured
   * EyeDiagramStorage. The capturer infers which sensors to track from the
   * storage object.
   ...
   *
   * The function detects rising edges based on worker_state and bins each
   * sensor value from measurements into storage relative to the last rising
   * edge timestamp.
   *
   ...
   */
  bool process_sample(const TimePoint &timestamp, int worker_state,
                      std::span<const float> measurements);
...
private:
   ...
   * This is built once at construction from the storage's
   * `original_sensor_indices`.
   */
